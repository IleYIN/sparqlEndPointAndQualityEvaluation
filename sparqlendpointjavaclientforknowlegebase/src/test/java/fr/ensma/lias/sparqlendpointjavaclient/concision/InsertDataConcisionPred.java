...
public class InsertDataConcisionPred {
	...
	public static void main(String[] args) throws MalformedURLException, ProtocolException, IOException {
		...
		for (int i = 0; i<30000; i++){
			...
			String Data = readData.readLineFile("resultFileSubclass.txt", 2, i+1);
	...
	private static void concisionPred(String nameClass) throws MalformedURLException, ProtocolException, IOException {
		
		String urladdress = "http://linkedgeodata.org/sparql/";
		String command = "SELECT distinct ?p WHERE {?s a lgdo:"+nameClass+". ?s ?p ?o . filter regex(?p, \"http://linkedgeodata.org/ontology/\")}  order by ?p";
		String resultfilename = "resultFileBufferConciPred.txt";

		
		requete(urladdress, command, resultfilename);
		
		List<DataConcision> list = new ArrayList<DataConcision>();
		insertDataString("resultFileBufferConciPred.txt", list);	
		
		LinkedList[] simiRow = new LinkedList[30000];
		
		PrintWriter pwSimi = new PrintWriter(new FileWriter("resultFileBufferConSimiPred.txt",true));
...
//		pwSimi.write("-----------"+"\r\n"+"numbergroup: "+numbergroup+"  numbersimitot: "+numbersimitot+" concision: "+concision);
		System.out.println(nameClass+"	"+concision);
		pwSimi.write(nameClass+"	"+concision+"\r\n");
		pwSimi.flush();
		pwSimi.close();
	}
