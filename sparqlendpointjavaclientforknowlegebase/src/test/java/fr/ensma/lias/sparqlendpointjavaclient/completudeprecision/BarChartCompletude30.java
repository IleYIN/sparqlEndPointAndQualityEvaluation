...
public class BarChartCompletude30 {
	ChartPanel frame1;
	public  BarChartCompletude30(int NUMBER, String[] strWant){
      ...
      CategoryAxis domainAxis=plot.getDomainAxis();         //水平底部列表
       domainAxis.setLabelFont(new Font("Calibri",Font.BOLD,20));         //水平底部标题
       domainAxis.setTickLabelFont(new Font("Calibri",Font.BOLD,20));  //垂直标题
       ValueAxis rangeAxis=plot.getRangeAxis();//获取柱状
       rangeAxis.setLabelFont(new Font("Calibri",Font.BOLD,20));
        chart.getLegend().setItemFont(new Font("Calibri", Font.BOLD, 20));
...
//         dataset.addValue(ratioprecision/completude, "nameSubclass", "namePred");
     	...
     	for (int m = 0; m<20; m++){
			...
			nameSuj[m] = rPred3.readLineFile("resultFilePredCompletude30.txt", 0, n);
			...
			for (int k = 0; k<NUMBER; k++ ){
				...
				ratioSujPred[k] = rPred1.readLineFile("resultFilePredCompletude30.txt", 1 + n, 2 + NUMBER + n, k, 1);//init 1 end 22 
				namePred[k] = rPred2.readLineFile("resultFilePredCompletude30.txt", 3, k + 2 + n);
...
public static void main(String args[]){
	...
	int NUMBER = 30;
	String[] str = {"source", "version", "opening_hours", "operator","addr%3Astreet"};
	frame.add(new BarChartCompletude30(NUMBER, str).getChartPanel());           //添加柱形图
