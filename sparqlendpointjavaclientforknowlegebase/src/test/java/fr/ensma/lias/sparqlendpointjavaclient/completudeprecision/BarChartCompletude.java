...
import fr.ensma.lias.sparqlendpointjavaclient.utils.ReaderRank2;
	...
	public class BarChartCompletude {
		ChartPanel frame1;
		public  BarChartCompletude(int NUMBER, String[] strWant){
			CategoryDataset dataset = getDataSet(NUMBER, strWant);
	      JFreeChart chart = ChartFactory.createBarChart3D(
	     		                 "Complétude pour la propriété \"version\" ", // 图表标题
	                          "Propriété", // 目录轴的显示标签
	                          "ratio", // 数值轴的显示标签
	                          dataset, // 数据集
	      ...
	      CategoryAxis domainAxis=plot.getDomainAxis();         //水平底部列表
	       domainAxis.setLabelFont(new Font("Calibri",Font.BOLD,25));         //水平底部标题
	       domainAxis.setTickLabelFont(new Font("Calibri",Font.BOLD,25));  //垂直标题
	       ValueAxis rangeAxis=plot.getRangeAxis();//获取柱状
	       rangeAxis.setLabelFont(new Font("Calibri",Font.BOLD,25));
	        chart.getLegend().setItemFont(new Font("Calibri", Font.BOLD, 25));
	        chart.getTitle().setFont(new Font("Calibri",Font.BOLD,25));//设置标题字体
...
//	         dataset.addValue(ratioprecision/completude, "nameSubclass", "namePred");
	     	...
	     	for (int m = 0; m<NUMBER; m++){
				...
				nameSuj[m] = rPred3.readLineFile("resultFilePredCompletude.txt", 0, n);
				...
				for (int k = 0; k<NUMBER; k++ ){
					...
					ratioSujPred[k] = rPred1.readLineFile("resultFilePredCompletude.txt", 1 + n, 2 + NUMBER + n, k, 1);//init 1 end 22 
					namePred[k] = rPred2.readLineFile("resultFilePredCompletude.txt", 3, k + 2 + n);
